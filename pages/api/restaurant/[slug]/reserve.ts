...
export default async function handler(
    ...
    const searchTimesWithTables = await findAvailableTables({
        ...
        const searchTimeWithTables = searchTimesWithTables.find((t) => {
            return t.date.getTime() === new Date(`${day}T${time}`).getTime();
        })

        if(!searchTimeWithTables){
            return res.status(400).json({
                errorMessage: "Not available! Cannot book!"
            });
        }


    // if(
    //     new Date(`${day}T${time}`).getTime() < new Date(`${day}T${restaurant.open_time}`) ||
    //     new Date(`${day}T${time}`).getTime() > new Date(`${day}T${restaurant.close_time}`)
    // ) {
    //     return res.status(400).json({
...
//  http://localhost:3000/api/restaurant/vivaan-fine-indian-cuisine-ottawa/reserve?day=2023-05-27&time=11:00:00.000Z&partySize=4