...
export default function SearchSideBar({locations, cuisines, searchParams, }: {locations: Location[]; cuisines: Cuisine[]; searchParams: {city?: string, cuisine?: string, price?: PRICE}; }){
               ...
               const prices =[{price: PRICE.CHEAP, label: "$", className: "border w-full text-reg text-center font-light p-2"},
                  {price: PRICE.REGULAR, label: "$$", className: "border w-full text-reg text-center font-light rounded-r p-2"},
  ...
  return(
        ...
        <div className="mt-3 pb-4">
          ...
          <div className="flex">
            ...
            <Link href={{pathname: "/search", query: { ...searchParams, price, },}}       
               className ={className} >
                {label}
              </Link>
              ))}
